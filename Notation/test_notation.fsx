...
for str in functions do
    ...
    i <- i + 1
    use renderer = new TeXRenderer(16f)
    renderer.Typeset hlist
    let size = renderer.MeasureSize hlist
    printfn "size: %i, %i" (int size.W) (int size.H)
...
let dt = (dt1 - dt0) / 1000L
Console.WriteLine $"dt: {float32 dt / 1000f}ms"    
